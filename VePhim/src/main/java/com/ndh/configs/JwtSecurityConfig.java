...
import com.ndh.filters.CustomAccessDeniedHandler;
import com.ndh.filters.JwtAuthenticationTokenFilter;
import com.ndh.filters.RestAuthenticationEntryPoint;
...
public class JwtSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests().antMatchers("/api/movies/**").permitAll();
         http.authorizeRequests().antMatchers("/api/showtimes/").permitAll();
        http.authorizeRequests().antMatchers("/api/showtimes/**").permitAll();
           http.authorizeRequests().antMatchers("/api/tickets/").permitAll();
        http.authorizeRequests().antMatchers("/api/tickets/**").permitAll();
        http.authorizeRequests().antMatchers("/api/users/").permitAll();
          http.authorizeRequests().antMatchers("/api/users/**").permitAll();
              http.authorizeRequests().antMatchers("/api/feedbacks/").permitAll();
...
//                .antMatchers(HttpMethod.DELETE, "/api/**").access("hasRole('ROLE_ADMIN') or hasRole('ROLE_USER')")
                ...
                .antMatchers(HttpMethod.POST, "/api/**").hasAnyAuthority("Admin","User")
